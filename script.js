/**
 * MoveFitRx Stable PoC - Build 7.3 (Fresh Start)
 */
...
function initializeState() {
    if (REFERRED_PATIENTS.length === 0) {
        ...
        REFERRED_PATIENTS.push({ name: 'Sarah Connor', diagnosisId: 'HYPT', regimenName: 'Cardio Vascular Health', matrixId: c1.matrixId, gymAccessCode: c1.gymAccessCode, status: 'PAID', createdAt: Date.now() - 432000000 });
        const c2 = MOCK_CREDENTIALS[1]; c2.used = true;
        REFERRED_PATIENTS.push({ name: 'Jessica Jones', diagnosisId: 'OSTE', regimenName: 'Bone Density & Balance', matrixId: c2.matrixId, gymAccessCode: c2.gymAccessCode, status: 'PENDING_PAYMENT', createdAt: Date.now() - 172800000 });
    }
}
...
function switchTab(tab) {
    ...
    if (tab === 'doctor') { populateDiagnosisDropdown(); renderDoctorPatientList(); }
}
...
function renderDoctorPatientList() {
    ...
    list.innerHTML = REFERRED_PATIENTS.map(p => {
        const dx = DIAGNOSES.find(d => d.id === p.diagnosisId)?.name || 'N/A';
        const color = p.status === 'PAID' ? 'border-green-500' : 'border-yellow-500';
        return `<div class="card bg-white border-l-4 ${color} p-4 mb-3 shadow-sm">
            <p class="text-lg font-bold">${p.name}</p>
            <p class="text-sm text-gray-600">DX: ${dx} | ID: ${p.matrixId}</p>
...
function handleReferral(e) {
    ...
    REFERRED_PATIENTS.unshift({ name: e.target.name.value, email: e.target.email.value, diagnosisId: dx.id, regimenName: dx.regimen, matrixId: cred.matrixId, gymAccessCode: cred.gymAccessCode, status: 'PENDING_PAYMENT', createdAt: Date.now() });
    renderDoctorPatientList();
    e.target.reset();
}

function initializeApp() {
    console.log("MoveFitRx System: Online");
    initializeState();
